...
def refinementRate(df, testVersion):
    ...
    """
    r = list()
    for n in range(testVersion-1):
        refRate = pwr(df.Elements[n]/(df.Elements[n+1]),1/3)
        r.append(refRate)
    r.append(np.mean(r))
    df['r'] = r
    return df

def threeEquations(simDf, nVar, infoDf, var):
    # Simplified method
    """
    ...
    pn = 1.7
    pm = 1.5
    cm = (pwr(r,1.7)*(s1-s2)-(s2-s3))/((pwr(r,1.7)-pwr(r,1.5)-pwr(r,3.2)+pwr(r,3))*pwr(delta,1.5))
    # cm = (r**(1.7)*(s1-s2)-(s2-s3))/((r**(1.7)-r**(1.5)-r**(3.2)+r**(3))*delta**(1.5))
    sc = ((pwr(r,1.7)*s1-s2)*(pwr(r,3.2)-pwr(r,3))-(pwr(r,1.7)*s2-s3)*(pwr(r,1.7)-pwr(r,1.5)))/((pwr(r,1.7)-1)*((pwr(r,3.2)-pwr(r,3))-(pwr(r,1.7)-pwr(r,1.5))))
    # sc = ((r**(1.7)*s1-s2)*(r**(3.2)-r**(3))-(r**(1.7)*s2-s3)*(r**(1.7)-r**(1.5)))/((r**(1.7)-1)*((r**(3.2)-r**(3))-(r**(1.7)-r**(1.5))))
    cn = (s1-sc-cm*pwr(delta,1.5))/(pwr(hstar,1.7))
    # cn = (s1-sc-cm*delta**(1.5))/(hstar**(1.7))

    # Errors
    Enum = dict()   # Numerical
    Enum[0] = cn*pwr(hstar,1.7)
    Enum[1] = cn*pwr(r,1.7)*pwr(hstar,1.7)
    Enum[2] = cn*pwr(r,3.4)*pwr(hstar,1.7)
    Emod = dict()   # Modelled
    Emod[0] = cm*pwr(delta,1.5)
    Emod[1] = cm*pwr(r,1.5)*pwr(delta,1.5)
    Emod[2] = cm*pwr(r,3)*pwr(delta,1.5)
    jj=0
    for ii in simDf:
        simDf[ii]['Sc'] = float(sc[var].mean()*(len(sc[var]) > 1) + sc[var][0]*(len(sc) == 1))
    ...
    cn = float(cn[var].mean()*(len(cn[var]) > 1) + cn[var][0]*(len(cn[var]) == 1))
    cm = float(cm[var].mean()*(len(cm[var]) > 1) + cm[var][0]*(len(cm[var]) == 1))

    # print("CN = {:.6f}\nCM = {:.6f}\nSc = {:.6f}".format(cn[0],cm[0],sc[0]))

    d = {'Order of Accuracy for the Numerical Error (Pn)': pn,
        'Order of Accuracy for the Modelled Error (Pm)': pm,
        'Mean Constant for Numerical report (Cn)': cn,
        'Mean Constant for Modelled report (Cm)': cm,
        'Maximum Mesh Division (x,y,z)': delta,
        'Temporal Resolution (h*)': hstar,
...
def fiveEquations(simDf, nVar, infoDf, var):
    # Full method
    """
    ...
    d = {'Order of Accuracy for the Numerical Error (Pn)': pn,
        'Order of Accuracy for the Modelled Error (Pm)': pm,
        'Mean Constant for Numerical report (Cn)': cn.mean(),
        'Mean Constant for Modelled report (Cm)': cm.mean(),
        'Delta': delta,
        'Hstar': hstar,
        'Mean Refinement Rate': r
        }
...
def run(testVersion, nVar, var, axis):
    ...
    ##                        "Choice: "))
    ## testVersion = 3*(testVersion==1) + 5*(testVersion==2)
    ...
    ## Simulated data
    if nVar == 1:
        simDf = threeEquations(simDf, nVar, infoDf, var)
    else:
        if testVersion == 3:
            simDf = threeEquations(simDf, nVar, infoDf, var)
        elif testVersion == 5:
            simDf = fiveEquations(simDf, nVar, infoDf, var)

    #     # Export Results to Excel
    #     d = {'Order of Accuracy for the Numerical Error (Pn)': report["pn"],
    #         'Order of Accuracy for the Modelled Error (Pm)': report["pm"],
    #         'Mean Constant for Numerical report (Cn)': report["cn"].mean(),
    #         'Mean Constant for Modelled report (Cm)': report["cm"].mean(),
    #         'Delta': report["delta"],
    #         'Hstar': report["hstar"],
    #         'Mean Refinement Rate': report["r"]
    #         }
    #     idx = [0]
    #     summary = pd.DataFrame(data=d, index=idx)
    #     xlsxFile = 'treatment/results/dataSummary.xlsx'
